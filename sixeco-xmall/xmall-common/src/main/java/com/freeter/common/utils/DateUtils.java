...
public class DateUtils {
	...
	/**
	 * 对日期（Date类型）Date + 时间（String类型）Time进行Date类型转换
	 * 
	 * @param date
	 * @param time
	 * @return
	 */
	public static Date convertDate(Date date, String time) {
		if (date == null || StringUtils.isEmpty(time)) {
			return null;
		}
		String dateStr = format(date);
		String dateTimeStr = dateStr.concat(" " + time);
		return stringToDate(dateTimeStr, DATE_TIME_PATTERN);
	}
}
